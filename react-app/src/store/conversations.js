...
const SET_ONE_PUBLISHED_CONVERSATION_MESSAGES = 'conversations/SET_ONE_PUBLISHED_CONVERSATION_MESSAGES'
...
export const setOnePublishedConversationMessages = (conversation) => {
  return {
    type: SET_ONE_PUBLISHED_CONVERSATION_MESSAGES,
    payload: conversation
  }
}
...
export const getPublishedConversationMessages = conversationId => async dispatch => {
  const response = await fetch(`/api/conversations/published/${conversationId}`)
  if (response.ok) {
    const conversation = await response.json();
    dispatch(setOnePublishedConversationMessages(conversation))
  }
}
...
const conversationReducer = (state = {}, action) => {
  ...
  switch (action.type) {
    ...
    case SET_ONE_CONVERSATION_MESSAGES:
      ...
      return newState;
    case SET_ONE_PUBLISHED_CONVERSATION_MESSAGES:
      if (!newState.published_messages) newState.published_messages = {}
      newState.published_messages = { ...newState.published_messages, ...action.payload };
      return newState;
